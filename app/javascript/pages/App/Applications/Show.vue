...
function reEvaluateWithAI() {
  if (isReEvaluating.value) return
  isReEvaluating.value = true
  
  router.post(`/app/applications/${props.application.id}/re_evaluate`, {}, {
    preserveScroll: true,
    onSuccess: () => {
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
          ...
          <UiCard v-if="application.coverLetterEvaluation">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-sm font-semibold text-zinc-900">AI Analysis</h3>
              <div class="flex items-center gap-2">
                ...
                <button
                  v-if="currentUser?.canWrite"
                  @click="reEvaluateWithAI"
                  :disabled="isReEvaluating"
                  class="p-1.5 text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 rounded transition-colors disabled:opacity-50"
                  title="Re-evaluate with AI"
                >
                  <svg v-if="isReEvaluating" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
          ...
          <!-- No evaluation yet but has answers -->
          <UiCard v-else-if="application.answers?.length > 0 && currentUser?.canWrite">
            <div class="flex items-center justify-between">
              <div>
                <h3 class="text-sm font-semibold text-zinc-900">AI Analysis</h3>
                <p class="text-xs text-zinc-500 mt-1">Evaluate application answers with AI</p>
              </div>
              <button
                @click="reEvaluateWithAI"
                :disabled="isReEvaluating"
                class="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-zinc-700 bg-zinc-100 hover:bg-zinc-200 rounded-lg transition-colors disabled:opacity-50"
              >
                <svg v-if="isReEvaluating" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                ...
                {{ isReEvaluating ? 'Evaluating...' : 'Evaluate with AI' }}
              </button>
            </div>
          </UiCard>
